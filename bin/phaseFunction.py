    ...
    for line in open(map, 'r'):
        ...
        if (key) in genewithhits.keys():
            ...
            if sRNA_len == phase_length:
                ...
                genewithhits[key]=str(no)+'\t'+sRNA+'\t'+line1[4]+'\t'+str(phase_length)
            else:
                genewithhits[key]=str(no)+'\t'+sRNA+'\t'+line1[4]+'\t'+str(sRNA_len)
        else:
...
def ParseBins(dic: dict, bins: dict):
    """Parse bins
...
def PhaseScoreAnalysis(tuple):
    ...
    binDic = ParseBins(mapDic['attribute'], bins)
    ...
    for i in filteredCluster:
        ...
        modifiedBinDic = ParseBins(mapDic['attribute'], phaseClusterBins)
