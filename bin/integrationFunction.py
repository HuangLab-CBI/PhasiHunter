...
def Gff3Tobed(gff3file, outfile):
    ...
    with open(gff3file, 'r') as fn:
        for line in fn:
            ...
            if line.startswith("#"):
                continue
            strand, feature, start, end, longstring = l[0], l[2], l[3], l[4], l[8]
            ...
            if anno == '':
                ...
                fo.write(f'{strand}\t{start}\t{end}\t{feature}\t{anno}\n')
    fo.close()
...
def SplitCluster(final_cluster: list, island_number=5, phase_length=21):
    ...
    for i in final_cluster[index:]:
        if (i - elem)%21 in condition_list and (i-elem) <= island_number * phase_length + 2:
            elem = list_.pop()
...
def SecondScaning(final_allsiRNA: dict, setting: tuple):
    ...
    for k in final_allsiRNA:
        ...
        candidate_cluster = SplitCluster(sorted_list)
                ...
                if e >= 4:
                    for length in list(sorted(pair_pos[e].keys()))[::-1]:
                        for tuple_ in pair_pos[length][e]:
                            ...
                            remainder = (tmp_pos - start_pos)%21
                            if remainder == 0:
                                end_pos = tmp_pos
                            elif remainder == 2:
                                end_pos = tmp_pos + 19
...
def GetPhaseScoreCluster(pcluster_dic):
    dic = nestedDic() 
    for i in pcluster_dic:
        for record_marker in pcluster_dic[i]:
            ...
            dic[i][record_marker] = [minium, maxium + 21]
    return dic
...
def GeneratingIntronInforFromGff(tmp_gff_bed):
    ...
    with open(tmp_gff_bed, 'r') as fn:
        ...
        for line in fn:
            l = line.strip().split("\t")
            Chr, start, end, feature, anno = l[0], int(l[1]), int(l[2]), l[3], l[4]
            ...
            if feature == 'exon':
                if former_feature == feature and former_fiter_anno == filter_anno:
                    ...
                    if region[0] > region[1]:
                        continue
                    intron = (Chr, region, 'intron', filter_anno + '--' + str(int(filter_order) - 1))
...
def GetPhasiRNAClusterBed(hout_pout, bedout, Type):
    fo = open(bedout, 'w')
    if Type == 'h':
        ...
        for geneid in hout_pout['pg']['phasiRNA']:
            for record in hout_pout['pg']['phasiRNA'][geneid]:
                ...
                fo.write(f'{geneid_}\t{sorted_region[0]}\t{str(int(sorted_region[-1]) + 21)}\n')
    fo.close()

def AnnoGdnaWithBedtools(tmp_hout_bed, tmp_gff_bed):
    fo = os.popen(f'bedtools intersect -a {tmp_hout_bed} -b {tmp_gff_bed} -wao | cut -f 1,2,3,7,8,9')
    return fo

def WringIntronInforToGff(intron_infor, tmp_gff_bed):
    with open(tmp_gff_bed, 'a+') as fo:
        for i in intron_infor:
            string = f'{str(i[0])}\t{str(i[1][0])}\t{str(i[1][1])}\t{str(i[2])}\t{str(i[3])}\n'
            fo.write(string)
...
def ParseBedAnno(hout_anno_io):
    ...
    for line in hout_anno_io:
        l = line.strip().split("\t")
        Chr, start, end, feature, anno, overlap = l[0], int(l[1]), int(l[2]), l[3], l[4], l[5]
        if overlap == '0':
            anno = 'Intergenic'
            feature = 'Other'
...
def FinalOut(tmp_out_list, relation_dic, fo):
    dic = defauldict_list()
...
def Integration_overlap(overlap_Intergenic_fo, fo_intergration_tmp):
